...
window.app = {
  ...
  playerModal(teamEnc, playerEnc) { const team=decodeURIComponent(teamEnc), player=decodeURIComponent(playerEnc); const stats=this.playerStats(player); $("modalBody").innerHTML=`<h2>${this.safe(player)}</h2><p>${this.safe(team)}</p><div class="quick-grid"><div><span>Runs</span><b>${stats.runs}</b></div><div><span>Balls</span><b>${stats.balls}</b></div><div><span>4s/6s</span><b>${stats.fours}/${stats.sixes}</b></div><div><span>Wkts</span><b>${stats.wkts}</b></div></div>`; $("modal").classList.add("show"); },
  closeModal(){ $("modal").classList.remove("show"); },
  playerStats(player){const s={runs:0,balls:0,fours:0,sixes:0,wkts:0}; this.allBatters(this.state).forEach(b=>{if(b.name===player){s.runs+=+b.r||0;s.balls+=+b.b||0;s.fours+=+b.f||0;s.sixes+=+b.s||0;}}); Object.values(this.state.inningsDetails||{}).forEach(i=>{const b=i.bowlerStats?.[player]; if(b)s.wkts+=+b.wkts||0;}); return s;},
  ...
  ballClass(x){const t=String(x);return t.includes('W')?'wicket':(t==='4'||t==='6'?'boundary':'');},
